...
% function handle for the Jacobian df/dx 
df = @(x) [1 0 0; 0 1 0; 0 0 1];

% function handle for the Jacobian dh/dx 
dh = @(x, ti, phi) [ 2*cos(conj(phi) + conj(ti)*conj(x(3)))         0                                                                   -2*sin(conj(phi) + conj(ti)*conj(x(3)))*conj(ti)*conj(x(1));
 2*cos(conj(x(2)))*cos(conj(phi) + conj(ti)*conj(x(3)))   -2*sin(conj(x(2)))*cos(conj(phi) + conj(ti)*conj(x(3)))*conj(x(1))  -2*cos(conj(x(2)))*sin(conj(phi) + conj(ti)*conj(x(3)))*conj(ti)*conj(x(1));
 2*sin(conj(x(2)))*cos(conj(phi) + conj(ti)*conj(x(3)))   2*cos(conj(x(2)))*cos(conj(phi) + conj(ti)*conj(x(3)))*conj(x(1))   -2*sin(conj(x(2)))*sin(conj(phi) + conj(ti)*conj(x(3)))*conj(ti)*conj(x(1))];
 
% Constructing the system postulated by the filter.
theSys = SigObsSys(f, h, wDistr, vDistr, initX, T, Phase_ang, [], dh);
...
opts = setPFOptions('NumPart', num_particles, 'ResampAlgo', 'fcn_ResampResid', 'ResampPeriod',1);

% constructing the particle filter
theFltr = PF_EKF(theSys, initDistr, opts);
